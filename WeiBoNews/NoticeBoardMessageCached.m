...
#import "NoticeBoardMessageCached.h"
#import "DataBaseManager.h"

@implementation NoticeBoardMessageCached
- (id) init {
    self = [super init];
    if (self) {
...
- (void)createTable {
    NSString * sql = @"CREATE TABLE IF NOT EXISTS MyChannel (id VARCHAR(64), type VARCHAR(2048) DEFAULT NULL, name VARCHAR(64) DEFAULT NULL, uicode VARCHAR(64))";
...
-(void)insertItem:(MenuInfo *)item
{
    ...
    NSString *sql=[NSString stringWithFormat:@"INSERT INTO MyChannel (id,type,name,uicode) VALUES (?,?,?,?,?)"];
    //插入也用executeUpdate方法
    if ([_db executeUpdate:sql,item.id,item.type,item.name, item.uicode]) {
        NSLog(@"插入成功");
    }
    else{
