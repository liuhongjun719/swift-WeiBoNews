...
#import "HJFullScreenPlayerView.h"
...
@interface HJFullScreenPlayerView () <UIGestureRecognizerDelegate,UIAlertViewDelegate>
...
@implementation HJFullScreenPlayerView
...
{
    static HJFullScreenPlayerView *playerView = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        playerView = [[HJFullScreenPlayerView alloc] init];
    });
    return playerView;
}
...
- (void)zf_controlView:(UIView *)controlView backAction:(UIButton *)sender
{
    if (ZFPlayerShared.isLockScreen) {
        ...
        if (!self.isFullScreen) {
            ...
            [self pause];
            if ([self.delegate respondsToSelector:@selector(hj_playerBackAction)]) { [self.delegate hj_playerBackAction]; }
...
- (void)zf_controlView:(UIView *)controlView downloadVideoAction:(UIButton *)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(hj_playerDownload:)]) {
        [self.delegate hj_playerDownload:urlStr];
    }
