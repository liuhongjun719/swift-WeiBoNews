...
/** 下载视频 */
- (void)hj_playerDownload:(NSString *)url;

@end

// 返回按钮的block
typedef void(^HJPlayerBackCallBack)(void);
// 下载按钮的回调
typedef void(^HJDownloadCallBack)(NSString *urlStr);

// playerLayer的填充模式（默认：等比例填充，直到一个维度到达区域边界）
typedef NS_ENUM(NSInteger, HJPlayerLayerGravity) {
    HJPlayerLayerGravityResize,           // 非均匀模式。两个维度完全填充至整个视图区域
    HJPlayerLayerGravityResizeAspect,     // 等比例填充，直到一个维度到达区域边界
    HJPlayerLayerGravityResizeAspectFill  // 等比例填充，直到填充满整个视图区域，其中一个维度的部分区域会被裁剪
};

@interface HJFullScreenPlayerView : UIView <ZFPlayerControlViewDelagate>
...
/** 设置代理 */
@property (nonatomic, weak) id<HJPlayerDelegate>   delegate;
/** 是否被用户暂停 */
...
@property (nonatomic, copy  ) HJPlayerBackCallBack goBackBlock __deprecated_msg("Please use ZFPlayerDelegate 'zf_playerBackAction' instead");
@property (nonatomic, copy  ) HJDownloadCallBack   downloadBlock __deprecated_msg("Please use ZFPlayerDelegate 'zf_playerDownload:' instead");
